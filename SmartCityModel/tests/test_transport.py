...
# =============================================================================
# ТЕСТЫ ДЛЯ КЛАССА PublicTransportVehicle
# =============================================================================
...
class TestPublicTransportVehicle(unittest.TestCase):
    ...
    def test_report_stop_passed_backward(self):
        ...
        result = self.vehicle.report_stop_passed(3)  # Меньше предыдущего

        self.assertIsNone(result)  # Не должно обновляться
    ...
    def test_update_passengers_valid(self):
        ...
        self.vehicle.update_passengers(60)  # Максимум
...
# =============================================================================
# ТЕСТЫ ДЛЯ КЛАССА TransportRoute
# =============================================================================
...
class TestTransportRoute(unittest.TestCase):
    ...
    def test_add_vehicle_different_route(self):
        ...
        self.assertEqual(len(self.route.vehicles), 0)  # Не должно добавиться

    def test_get_status(self):
        """Получение статуса маршрута."""
        # Добавляем 2 транспорта
        for i in range(2):
...
# =============================================================================
# ТЕСТЫ ДЛЯ КЛАССА TransportMonitoringSystem
# =============================================================================
...
class TestTransportMonitoringSystem(unittest.TestCase):
    ...
    def test_register_route_no_duplicate_stops(self):
        ...
        # Регистрируем первый маршрут
        self.tms.register_route(self.route)

        # Создаём второй маршрут с той же остановкой
        route2 = TransportRoute("R2", [RouteStop(self.stop1, 0)])
        self.tms.register_route(route2)

        # Остановка должна быть одна
        self.assertEqual(len(self.tms.physical_stops), 2)  # stop1 и stop2
    ...
    def test_register_vehicle_without_route(self):
        ...
        # Не регистрируем маршрут заранее
        self.tms.register_vehicle(self.vehicle)

        self.assertIn(self.vehicle.device_id, self.tms.vehicles)
        # Транспорт не должен добавиться в маршрут, т.к. его нет в tms.routes

    def test_calculate_eta_same_stop(self):
        """ETA = 0, если транспорт уже на остановке."""
        self.vehicle.report_stop_passed(1)  # Транспорт на индексе 1
        target_stop = self.route_stops[1]  # Целевая остановка тоже индекс 1
    ...
    def test_calculate_eta_future_stop(self):
        ...
        self.vehicle.report_stop_passed(0)  # Транспорт на старте
        target_stop = self.route_stops[1]  # Целевая - следующая

        with patch('random.randint', return_value=1):  # Фиксируем случайность
            eta = self.tms.calculate_eta(self.vehicle, target_stop, self.route)

        # 1 остановка * 3 мин + 1 мин случайная = 4 мин
        self.assertEqual(eta, 4.0)

    def test_calculate_eta_past_stop(self):
        """ETA = None, если остановка уже пройдена."""
        self.vehicle.report_stop_passed(1)  # Транспорт прошёл индекс 1
        target_stop = self.route_stops[0]  # Целевая - предыдущая
    ...
    def test_calculate_eta_unknown_position(self):
        ...
        # _last_passed_stop_index = -1 по умолчанию
    ...
    def test_get_arrival_info_with_vehicle(self):
        ...
        # Табло должно обновиться
        self.assertIn("мин", self.stop2._display_message)


# =============================================================================
# ТЕСТЫ ДЛЯ КЛАССА SmartTrafficLight
# =============================================================================
...
class TestSmartTrafficLight(unittest.TestCase):
    ...
    def test_get_traffic_request_high_flow(self):
        ...
        self.flow_sensor.get_status.return_value = 20  # > 15
        self.pedestrian_sensor.get_status.return_value = 4
    ...
    def test_get_traffic_request_pedestrians_waiting(self):
        ...
        self.flow_sensor.get_status.return_value = 5
        self.pedestrian_sensor.get_status.return_value = 10  # > intensity
    ...
    def test_get_traffic_request_default(self):
        ...
        self.flow_sensor.get_status.return_value = 3
        self.pedestrian_sensor.get_status.return_value = 1  # < intensity
...
# =============================================================================
# ТЕСТЫ ДЛЯ КЛАССА Intersection
# =============================================================================

class TestIntersection(unittest.TestCase):

    def setUp(self):
        # Создаём моки светофоров
    ...
    def test_regulate_intersection_emergency_priority(self):
        ...
        # Светофор N имеет аварийный приоритет
        ...
        self.assertTrue(warning)  # Инцидент = предупреждение
        # Север должен быть зелёным, юг - красным (конфликтных направлений нет при 2 светофорах)
        # Но при 2 светофорах conflict_map пустой, поэтому оба могут быть зелёными
        # Проверяем, что set_color был вызван
    ...
    def test_regulate_intersection_four_way_conflict(self):
        ...
        # Все светофоры с обычным приоритетом, лидер - север
        ...
        # Проверяем, что set_color вызывался для всех светофоров
        for light in [self.light_n, self.light_s, self.light_e, self.light_w]:
            light.set_color.assert_called()


# =============================================================================
# ТЕСТЫ ДЛЯ КЛАССА TrafficManager
# =============================================================================
...
class TestTrafficManager(unittest.TestCase):
    ...
    def setUp(self):
        ...
        # Создаём моки для перекрёстков и светофоров
    ...
    def test_link_stop_duplicate_direction(self):
        ...
        # Первая связь успешна
        self.tm.link_stop_to_intersection(stop1, "INT_1", Direction.NORTH)

        # Вторая связь с тем же направлением должна вызвать ошибку
        with self.assertRaises(TransportException) as context:
    ...
    def test_prioritize_public_transport_non_public_vehicle(self):
        ...
        # Создаём маршрут с личным авто
        vehicle = Mock(spec=PublicTransportVehicle)
        vehicle.vehicle_type = VehicleType.CAR  # Не общественный
        vehicle.route_id = "R1"
    ...
    def test_prioritize_public_transport_linked_stop(self):
        ...
        # Создаём общественный транспорт
        vehicle = Mock(spec=PublicTransportVehicle)
        ...
        # Проверяем, что detect_event был вызван для уведомления светофора
